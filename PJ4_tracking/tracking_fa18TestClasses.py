...
class DoubleInferenceAgentTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        disp = self.question.getDisplay()
        pac = DoubleInferenceAgent(inferenceFunction, moves, ghosts, grades, self.seed, disp, self.inference,
                                   elapse=self.elapse,
...
class DoubleInferenceAgent(bustersAgents.BustersAgent):
    ...
    def __init__(self, inference, refSolution, ghostAgents, grades, seed, disp, func, elapse=True, observe=True,
    ...
    def distCompare(self, dist, refDist):
        ...
        if l2 > self.L2Tolerance:
            if self.errors == 0:
                ...
                detail = '\n'.join(list(map(lambda x: '%9s:%5s%-25s%-25s' % (x, '', dist[x], refDist[x]),
